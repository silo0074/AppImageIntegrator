...
from Logic import DesktopFileHandler, AppImageParser, MimeWorker, InteractiveHelp, IconThumbnail
import config
import stylesheet
...
class AboutDialog(QDialog):
    def __init__(self, parent=None):
        ...
        pixmap = QPixmap("icons/AppIntegrator.png")
...
class Integrator(QMainWindow):
    ...
    def generate_mime_list(self, group_name, container, source, field_name):
        ...
        group_name: Dict[str, MimeGroupWidget] = {}
    ...
    def update_mimetype_field(self, group_name, source, field_name):
        # Find the MimeType QLineEdit in the dynamic form
        ...
        # Get currently typed mimes from the QLineEdit
        current_text = mime_line_edit.text()

        # Split by semicolon and clean up
        current_mimes = set(m.strip() for m in current_text.split(';') if m.strip())

        # Get all mimes that ARE NOT in our checkboxes (these are the 'User Typed' ones)
        all_known_mimes = set()
        for gw in group_name.values():
            for cb in gw.checkboxes:
                all_known_mimes.add(cb.text())

        user_custom_mimes = current_mimes - all_known_mimes

        # Get all mimes that ARE currently checked in the UI
        selected_from_ui = set()
        for gw in group_name.values():
            selected_from_ui.update(gw.selected_mimes())

        # Combine custom mimes + checked mimes
        final_mimes = sorted(list(user_custom_mimes | selected_from_ui))

        # Update the line edit with a semicolon-separated string
        mime_line_edit.setText(";".join(final_mimes) + (";" if final_mimes else ""))
    ...
    def createDesktop(self):
        ...
        try:
            ...
            if self.ui.cbMakeDefault.isChecked() and gui_data.get("MimeType"):
                print("set as default for selected MIME types")
    ...
    def load_icons(self):
        ...
        # Clear existing layout from Designer
        existing_layout = self.ui.scrollAreaWidgetContents_2.layout()
        if existing_layout:
            self.clear_layout(existing_layout)

        # Assign the new FlowLayout
    ...
    def changeMIMEicon(self):
        ...
        if action == "create":
            self.ui.status.showMessage("MIME icon changed successfully!", 5000)
        else:
            self.ui.status.showMessage("Removed icon override for selected MIME", 5000)
